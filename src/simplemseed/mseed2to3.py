...
def mseed2to3(ms2: MiniseedRecord) -> MSeed3Record:
    ...
    ms2H = ms2.header
    ms3Header.flags = ((ms2H.actFlag & 1) * 2
       + (ms2H.ioFlag & 64 ) * 4
       + (ms2H.qualFlag & 16) * 8)
    ...
    ms3Header.sampleRatePeriod = ms2.header.sampleRate if ms2.header.sampleRate >= 1 else (-1.0 / ms2.header.sampleRate)
    ...
    if ms2.encodedData is not None:
        ...
        data = EncodedDataSegment(ms3Header.encoding,
                                    dataBytes,
