...
def encodeSteim2FrameBlock(samples: list[int], frames: int =0, bias: int = 0) -> SteimFrameBlock:
    if (len(samples) == 0):
    ...
    frameBlock.addEncodedWord(samples[len(samples)-1],0,0) # X(N) -- last sample value
    #
    ...
    while(sampleIndex < len(samples)):
        ...
        if nbits == 4:
            ...
            raise SteimException("Unable to encode " + nbits + " bit difference in Steim2 format")
...
def bitsForPack(minbits: list[int], points_remaining: int):
    if (points_remaining >= 7 and
        (minbits[0] <= 4) and (minbits[1] <= 4) and
        (minbits[2] <= 4) and (minbits[3] <= 4) and
        (minbits[4] <= 4) and (minbits[5] <= 4) and
        (minbits[6] <= 4)):
        return 4
    if (points_remaining >= 6 and
        (minbits[0] <= 5) and (minbits[1] <= 5) and
        (minbits[2] <= 5) and (minbits[3] <= 5) and
        (minbits[4] <= 5) and (minbits[5] <= 5)):
        return 5
    if (points_remaining >= 5 and
        (minbits[0] <= 6) and (minbits[1] <= 6) and
        (minbits[2] <= 6) and (minbits[3] <= 6) and
        (minbits[4] <= 6)):
        return 6
    if (points_remaining >= 4 and
        (minbits[0] <= 8) and (minbits[1] <= 8) and
        (minbits[2] <= 8) and (minbits[3] <= 8)):
        return 8
    if (points_remaining >= 3 and
        (minbits[0] <= 10) and (minbits[1] <= 10) and
        (minbits[2] <= 10)):
        return 10
    if (points_remaining >= 2 and
        (minbits[0] <= 15) and (minbits[1] <= 15)):
        return 15
    if (points_remaining >= 1 and
        (minbits[0] <= 30)):
        return 30
    return 32
