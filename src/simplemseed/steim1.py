...
def encodeSteim1(samples: list[int], frames: int=0, bias: int = 0, offset: int = 0) -> bytearray:
    """
...
def encodeSteim1FrameBlock(samples: list[int], frames: int=0, bias: int = 0, offset: int = 0) -> SteimFrameBlock:
    ...
    if (offset >= len(samples)):
        raise SteimException("Offset bigger than samples array: "+offset+" >= "+len(samples))
